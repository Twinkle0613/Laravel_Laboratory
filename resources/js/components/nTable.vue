<template>
    <div class="row container-fluid">
        <div class=" col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            ...
                            <button type="button" class="btn btn-tool"  data-toggle="add" title="Download">
                                <i class="fas fa-download"></i> EXCEL</button>
                            <!-- <button type="button" class="btn btn-tool" data-toggle="modal" data-target="#action-modal" @click="showModal('add')" > -->
                            <button type="button" class="btn btn-tool"  data-target="#action-modal" @click="showModal('add')" >
                                <i class="fas fa-lg fa-plus-square"></i></button>
                    ...
                    <div class="card-body p-0">
                        <table id="user_table" class="table table-striped projects">
                            <thead class="text-center">
                                <tr>
                                    <th v-for="field in table_fields" :key="field.key" :style="{'width':field.width}">
                                        {{field.label}}
                                    </th>
                                    <th>
                                        Action
                            ...
                            <tbody>
                                    <tr v-for="(data) in table_data" :key="data.id" class="text-center">
                                        <th v-for="field in table_fields" :key="field.key">
                                            {{data[field.key]}}
                                        </th>
                                        <td class="project-actions text-right">
                                            <!-- <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#action-modal" @click="showModal('view',data.id)" > -->
                                            <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#action-modal"  >
                    ...
                    <div class="card-footer clearfix">
                        <ul class="pagination pagination-sm m-0 float-left">
                            <li class="page-item"><a class="page-link" @click="updatePageNo(first_page)">&laquo;</a></li>
                            <li v-for="(n) in meta.last_page" :key='n' class="page-item" @click="updatePageNo(n)"><a class="page-link" href="#">{{n}}</a></li>
                            <li class="page-item"><a class="page-link"  @click="updatePageNo(meta.last_page)">&raquo;</a></li>
                        </ul>

                        <select class="form-control select2 float-right form-control-sm"  v-model="per_page" @change="updatePerPage" style="width: 100px;">
                            ...
                            <div class="modal-header">
                                ...
                                <div class="modal-body">
                                 ...
                                 <div class="modal-footer justify-content-between">
                                    ...
                                    <button type="button" class="btn btn-primary" data-dismiss="modal" v-on:click="postData(modal.mode)">{{modal.button_label}}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <b-modal v-model="modal.show" @hidden="resetModal" @show="resetModal" ref="action-modal" size="lg" :title-html="modal.title" title-tag="h4">
                     <n-form ref="nForm" :inputs_info="inputs_info" @update="nFormUpdate" ></n-form>
                     <template v-slot:modal-footer>
                         <div class="w-100">
                            <b-button type="button" class="btn btn-default float-left" @click="modal.show=false" >Close</b-button>
                            <b-button type="button" class="btn btn-primary float-right" variant="primary"  v-on:click="postData(modal.mode)">{{modal.button_label}}</b-button>
                         </div>
                     </template>
                </b-modal>
...
<script>
    ...
    export default {
        ...
        mounted() {
            let self = this.$refs.nForm;
            $('#action-modal').on('hidden.bs.modal', function (e) {
               self.inputs_info.forEach(element => {
                    self.inputs[element.key]=null;
                });
           });
        ...
        methods:{
            ...
            postData(mode){
                this.$http.post(this.url_path,this.inputs)
                .then((response) => {
                    Swal.fire({
                    icon: 'success',
